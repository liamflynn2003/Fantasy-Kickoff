...
{
    // Only clear on Monday
    if (System.DateTime.Now.DayOfWeek == System.DayOfWeek.Monday)
    {
        string lastClearDate = PlayerPrefs.GetString("LastCacheClearDate", "");
        ...
        if (lastClearDate != today)
        {
            if (File.Exists(cacheFilePath))
            {
                try
                {
                    ...
                    Debug.Log("Weekly cache cleared on Monday.");
                }
                catch (System.Exception e)
                {
                    Debug.LogError($"Failed to delete cache: {e.Message}");
                }
            }

            PlayerPrefs.SetString("LastCacheClearDate", today);
            PlayerPrefs.Save();
        }
    }
}
