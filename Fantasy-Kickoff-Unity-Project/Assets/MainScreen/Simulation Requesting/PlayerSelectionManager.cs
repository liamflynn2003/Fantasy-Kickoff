...
public class PlayerSelectionManager : MonoBehaviour
{
    ...
    private bool MultiplayerSelectingTeamOne => PlayerSessionInfo.multiplayerIsTeamOne;
    ...
    if (string.IsNullOrEmpty(player.name))
{
    ...
    if (isTeamOne)
    {
        ...
        Debug.Log($"Final currentPOS for {player.name}: {player.currentPOS}");
    }
    else
    {
...
public void MultiplayerAssignPlayer(int positionIndex, PlayerListManager.PlayerData player)
{
    if (!multiplayerMode)
    {
        Debug.LogError("Tried to use MultiplayerAssignPlayer in non-multiplayer mode.");
        return;
    }

    AssignPlayer(positionIndex, player, PlayerSessionInfo.multiplayerIsTeamOne);
}
    ...
    public Dictionary<int, PlayerListManager.PlayerData> GetMultiplayerSelectedTeam()
    {  
    if (!multiplayerMode)
    {
        return null;
    ...
    return GetSelectedTeam(PlayerSessionInfo.multiplayerIsTeamOne);
    }

public bool IsMultiplayerTeamValid()
{
    if (!multiplayerMode)
    {
        return false;
    ...
    return IsTeamValid(PlayerSessionInfo.multiplayerIsTeamOne);
}
