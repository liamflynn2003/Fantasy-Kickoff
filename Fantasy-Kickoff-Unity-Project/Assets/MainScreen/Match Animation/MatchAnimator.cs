...
public class MatchAnimator : MonoBehaviour
{
    ...
    public float minTimeBetweenIterations = 0.000001f;
    public float maxTimeBetweenIterations = 0.05f;
    ...
    // Clock UI
    public TMP_Text matchHalfText;
    public TMP_Text matchClockText;
    ...
    private float matchMinutes = 0f;
    private const float minutesPerIteration = 0.009f; // 90 mins / 10,000 iterations
...
IEnumerator PlayMatch()
{
    while (currentIteration < maxIterations)
    {
        ...
        UpdateMatchClock(currentIteration);
...
// Decrease time between iterations (faster)
public void FastForward()
{
    timeBetweenIterations = Mathf.Max(minTimeBetweenIterations, timeBetweenIterations / 2f);
    Debug.Log($"Fast Forward: {timeBetweenIterations}");
}

// Increase time between iterations (slower)
public void SlowDown()
{
    timeBetweenIterations = Mathf.Min(maxTimeBetweenIterations, timeBetweenIterations * 2f);
    Debug.Log($"Slow Down: {timeBetweenIterations}");
}
    ...
    void UpdateScore(int iteration)
{
    ...
    void UpdateMatchClock(int iteration)
{
    matchMinutes = iteration * minutesPerIteration;
    int minutes = Mathf.FloorToInt(matchMinutes);
    int seconds = Mathf.FloorToInt((matchMinutes - minutes) * 60f);

    matchClockText.text = $"{minutes:00}:{seconds:00}";

    if (minutes < 46)
        matchHalfText.text = "Half: 1";
    else
        matchHalfText.text = "Half: 2";
}
